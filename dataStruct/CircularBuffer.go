...
func NewCircularBuffer[T any](size int) (CircularBuffer[T],error) {
    if size<=0 {
        return CircularBuffer[T]{},customerr.ValOutsideRange(
            fmt.Sprintf("Size of buffer must be >0 | Have: %d",size),
        );
    }
...
// Appends the supplied values to the circular buffer. This function will never 
// return an error.
...
func (c *CircularBuffer[T])getFullError() error {
    return Full(fmt.Sprintf("Circular buffer size: %d",len(c.vals)));
}
